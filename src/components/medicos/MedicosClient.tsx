    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-lg font-semibold text-gray-900">{isEdit ? 'Editar médico' : 'Añadir médico'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" required autoFocus className="input" value={nombre}
              onChange={e => setNombre(e.target.value)} placeholder="Dr. García López" />
          </div>
          <div>
...
function DoctorRow({ doctor, onEdit, onCalendario, onConfig, onRefresh }: {
  doctor: Doctor; onEdit: () => void; onCalendario: () => void; onConfig: () => void; onRefresh: () => void
}) {
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          {doctor.fecha_inicio_contrato && (
            <span className="text-xs text-gray-400">desde {doctor.fecha_inicio_contrato.split('-').reverse().join('/')}</span>
          )}
        </div>
      </div>
      {!confirmDelete ? (
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={handleDelete} disabled={loading} className="text-xs font-medium text-red-600 hover:text-red-800">Sí</button>
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd} className="btn-primary"><Plus size={16} />Añadir médico</button>
      </div>
        ...
        <div className="space-y-2 mb-6">
          {activos.map(d => (
            <DoctorRow key={d.id} doctor={d}
              onEdit={() => openEdit(d)} onCalendario={() => setCalDoctor(d)}
              onConfig={() => setCfgDoctor(d)} onRefresh={() => router.refresh()} />
          ))}
        </div>
      )}
      ...
      {showModal && <DoctorModal doctor={editDoctor} onClose={closeModal} onSaved={onSaved} />}
      {calDoctor && <CalendarioModal doctor={calDoctor} onClose={() => setCalDoctor(undefined)} />}
      {cfgDoctor && (
        <DoctorConfigModal doctor={cfgDoctor}
