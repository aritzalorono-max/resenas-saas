...
function RegistroForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (email !== emailConf) { setError('Los correos electrónicos no coinciden.'); return }
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          {done ? (
            <div className="text-center py-4">
              ...
              <p className="text-gray-400 text-xs">Revisa también la carpeta de spam.</p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="email" required className="input" value={email}
                    onChange={e => setEmail(e.target.value)} placeholder="tu@hospital.eus" autoComplete="email" />
                </div>
                <div>
                  <label className="label">Confirmar correo electrónico</label>
                  <input type="email" required className="input" value={emailConf}
                    onChange={e => setEmailConf(e.target.value)} placeholder="tu@hospital.eus" autoComplete="off" />
                </div>
                <div>
                ...
                <button type="submit" disabled={loading || !terms} className="btn-primary w-full justify-center py-2.5 mt-2">
