  ...
  return (
    <article>
      ...
      <Section title={locale === "es" ? "1. Información general" :
        ...
        <p>
          {locale === "es" ? <>El presente documento establece los Términos y Condiciones de uso (en adelante, &quot;las Condiciones&quot;) que regulan el acceso y la utilización del servicio <strong>ReseñasYa</strong>, disponible en{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (en adelante, &quot;la Plataforma&quot;), titularidad de <strong>{COMPANY}</strong>, empresa debidamente constituida conforme a la legislación española, con CIF <strong>B-95612958</strong>, domicilio social en <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, inscrita en el Registro Mercantil de Vizcaya, Tomo 5138, Folio 19, Inscripción 1.ª, Hoja BI-56789, y correo electrónico de contacto:{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</> :
           locale === "en" ? <>This document sets out the Terms and Conditions of Use (hereinafter, &quot;the Conditions&quot;) governing access to and use of the <strong>ReseñasYa</strong> service, available at{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (hereinafter, &quot;the Platform&quot;), owned by <strong>{COMPANY}</strong>, duly incorporated under Spanish law, with tax ID (CIF) <strong>B-95612958</strong>, registered office at <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, registered in the Commercial Register of Biscay, Volume 5138, Sheet 19, Entry 1, Folio BI-56789, and contact email:{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</> :
           locale === "fr" ? <>Le présent document établit les Conditions Générales d&apos;Utilisation (ci-après, &quot;les Conditions&quot;) régissant l&apos;accès et l&apos;utilisation du service <strong>ReseñasYa</strong>, disponible à l&apos;adresse{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (ci-après, &quot;la Plateforme&quot;), appartenant à <strong>{COMPANY}</strong>, société dûment constituée conformément à la législation espagnole, avec numéro fiscal (CIF) <strong>B-95612958</strong>, siège social à <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, inscrite au Registre du Commerce de Biscaye, et e-mail de contact :{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</> :
           locale === "de" ? <>Dieses Dokument legt die Allgemeinen Geschäftsbedingungen (nachfolgend &quot;Bedingungen&quot;) fest, die den Zugang zu und die Nutzung des Dienstes <strong>ReseñasYa</strong> unter{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (nachfolgend &quot;die Plattform&quot;) regeln, im Eigentum von <strong>{COMPANY}</strong>, ordnungsgemäß nach spanischem Recht gegründet, mit Steuer-ID (CIF) <strong>B-95612958</strong>, eingetragene Adresse: <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, und Kontakt-E-Mail:{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</> :
           locale === "it" ? <>Il presente documento stabilisce i Termini e Condizioni d&apos;Uso (di seguito, &quot;le Condizioni&quot;) che regolano l&apos;accesso e l&apos;utilizzo del servizio <strong>ReseñasYa</strong>, disponibile all&apos;indirizzo{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (di seguito, &quot;la Piattaforma&quot;), di proprietà di <strong>{COMPANY}</strong>, società debitamente costituita ai sensi della legislazione spagnola, con codice fiscale (CIF) <strong>B-95612958</strong>, sede legale presso <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, e indirizzo e-mail di contatto:{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</> :
                             <>O presente documento estabelece os Termos e Condições de Utilização (doravante, &quot;as Condições&quot;) que regulam o acesso e a utilização do serviço <strong>ReseñasYa</strong>, disponível em{" "}<a href={APP_URL} className="text-brand-600 hover:underline">{APP_URL}</a> (doravante, &quot;a Plataforma&quot;), propriedade de <strong>{COMPANY}</strong>, empresa devidamente constituída ao abrigo da legislação espanhola, com NIF (CIF) <strong>B-95612958</strong>, sede social em <strong>Avda. Ribera de Axpe 11, 2D - 202, 48950 Erandio (Bizkaia)</strong>, e e-mail de contacto:{" "}<a href={`mailto:${EMAIL}`} className="text-brand-600 hover:underline">{EMAIL}</a>.</>
          }
        </p>
        <p>
          {locale === "es" ? <>La marca <strong>ReseñasYa</strong> es titularidad de <strong>{COMPANY}</strong>, registrada en la Oficina Española de Patentes y Marcas bajo la denominación <em>Buy &amp; Click</em>.</> :
           locale === "en" ? <>The <strong>ReseñasYa</strong> trademark is owned by <strong>{COMPANY}</strong>, registered at the Spanish Patent and Trademark Office under the name <em>Buy &amp; Click</em>.</> :
           locale === "fr" ? <>La marque <strong>ReseñasYa</strong> est la propriété de <strong>{COMPANY}</strong>, enregistrée à l&apos;Office Espagnol des Brevets et des Marques sous la dénomination <em>Buy &amp; Click</em>.</> :
           locale === "de" ? <>Die Marke <strong>ReseñasYa</strong> ist Eigentum von <strong>{COMPANY}</strong>, eingetragen beim Spanischen Patent- und Markenamt unter dem Namen <em>Buy &amp; Click</em>.</> :
           locale === "it" ? <>Il marchio <strong>ReseñasYa</strong> è di proprietà di <strong>{COMPANY}</strong>, registrato presso l&apos;Ufficio Spagnolo dei Brevetti e dei Marchi con la denominazione <em>Buy &amp; Click</em>.</> :
                             <>A marca <strong>ReseñasYa</strong> é propriedade de <strong>{COMPANY}</strong>, registada no Instituto Nacional de Propriedade Industrial espanhol sob a denominação <em>Buy &amp; Click</em>.</>
          }
        </p>
        <p>
...
function SubscriptionSection({ locale, email, billingPath }: { locale: string; email: string; billingPath: string }) {
  const labels = {
    es: {
      nature: "Naturaleza del servicio:", natureDesc: "ReseñasYa funciona exclusivamente mediante suscripción mensual de pago. No existe periodo de prueba gratuito salvo indicación expresa en la página de precios.",
    ...
    en: {
      nature: "Nature of the service:", natureDesc: "ReseñasYa operates exclusively on a monthly paid subscription basis. There is no free trial period unless expressly indicated on the pricing page.",
    ...
    fr: {
      nature: "Nature du service :", natureDesc: "ReseñasYa fonctionne exclusivement par abonnement mensuel payant. Il n'y a pas de période d'essai gratuite sauf indication expresse sur la page des tarifs.",
    ...
    de: {
      nature: "Art des Dienstes:", natureDesc: "ReseñasYa funktioniert ausschließlich im Rahmen eines monatlichen kostenpflichtigen Abonnements. Es gibt keine kostenlose Testphase, sofern nicht ausdrücklich auf der Preisseite angegeben.",
    ...
    it: {
      nature: "Natura del servizio:", natureDesc: "ReseñasYa funziona esclusivamente tramite abbonamento mensile a pagamento. Non esiste un periodo di prova gratuito salvo indicazione espressa nella pagina dei prezzi.",
    ...
    pt: {
      nature: "Natureza do serviço:", natureDesc: "ReseñasYa funciona exclusivamente mediante subscrição mensal paga. Não existe período de teste gratuito salvo indicação expressa na página de preços.",
