...
-- ─── 4. Añadir active_team_id a perfiles ──────────────────────────────────────

ALTER TABLE guardias_profiles
  ADD COLUMN IF NOT EXISTS active_team_id uuid REFERENCES guardias_teams(id);

-- ─── 5. Añadir team_id a todas las tablas existentes ─────────────────────────

ALTER TABLE guardias_doctor_profiles  ADD COLUMN IF NOT EXISTS team_id uuid REFERENCES guardias_teams(id);
ALTER TABLE guardias_assignments       ADD COLUMN IF NOT EXISTS team_id uuid REFERENCES guardias_teams(id);
...
ALTER TABLE guardias_rules_config      ADD COLUMN IF NOT EXISTS team_id uuid REFERENCES guardias_teams(id);
...
CREATE INDEX IF NOT EXISTS idx_assignments_team      ON guardias_assignments(team_id);
...
-- Teams: cualquier miembro activo puede ver su equipo
CREATE POLICY "teams_select" ON guardias_teams
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM guardias_team_members
      WHERE team_id = guardias_teams.id
        AND profile_id = auth.uid()
        AND status = 'active'
    )
  );

-- Teams: solo el creador puede actualizar
...
-- Teams: cualquier usuario autenticado puede crear
CREATE POLICY "teams_insert" ON guardias_teams
  FOR INSERT TO authenticated WITH CHECK (true);

-- Team members: miembros activos pueden ver su equipo
...
-- Invitations: gestores pueden ver/crear; invitado puede ver la suya por token
...
-- ─── 7. RLS – Actualizar políticas existentes con team_id ─────────────────────

-- Helper: ¿es el usuario gestor del equipo?
-- (usado en políticas de las tablas con team_id)
...
CREATE POLICY "asgn_select" ON guardias_assignments
  FOR SELECT TO authenticated
  USING (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "asgn_insert" ON guardias_assignments
  FOR INSERT TO authenticated
  WITH CHECK (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "asgn_update" ON guardias_assignments
  FOR UPDATE TO authenticated
  USING (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "asgn_delete" ON guardias_assignments
  FOR DELETE TO authenticated
  USING (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "rc_select" ON guardias_rules_config
  FOR SELECT TO authenticated
  USING (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "rc_update" ON guardias_rules_config
  FOR UPDATE TO authenticated
  USING (
    team_id IN (
      SELECT team_id FROM guardias_team_members
...
CREATE POLICY "rc_insert" ON guardias_rules_config
  FOR INSERT TO authenticated WITH CHECK (true);
...
-- ─── 8. Auto-migrar datos existentes ──────────────────────────────────────────
-- Crea un equipo para el primer admin existente y asocia todos los datos huérfanos
...
BEGIN
  ...
  IF admin_id IS NOT NULL THEN
    ...
    -- Asociar todos los datos huérfanos al nuevo equipo
    UPDATE guardias_doctor_profiles  SET team_id = new_team WHERE team_id IS NULL;
    UPDATE guardias_assignments       SET team_id = new_team WHERE team_id IS NULL;
    ...
    UPDATE guardias_rules_config      SET team_id = new_team WHERE team_id IS NULL;
